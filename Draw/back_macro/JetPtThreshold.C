...
void JetPtThreshold(){
  ...
  TString sPathNu[] = {"/home/cuipengyao/study/pPb/Local/ThisAna/Uncertainty/JE/Loose", "/home/cuipengyao/study/pPb/Local/ThisAna/Uncertainty/JE/Tight"};
  TString sFileNu[] = {"AnalysisOutputs_Results.root", "AnalysisOutputs_Results.root"};
  TString sListNu[] = {"listResults_Lambda_Default_JC_0100", "listResults_Lambda_Default_JC_0100"};
  TString sHistNu[] = {"hNco", "hNco"};

  TString sPathDe = "/home/cuipengyao/study/pPb/Local/ThisAna/JEBins";
  TString sFileDe = "AnalysisOutputs_Results.root";
  TString sListDe = "listResults_Lambda_Default_JC_0100";
  TString sHistDe = "hNco";
  
  TString sLeg[] = {"#it{p}_{T, jet} > 9", "#it{p}_{T, jet} > 11", "#it{p}_{T, jet} > 10"};

  auto sLatex(Form("Lambda in JC with differnt jet #it{p}_{T} cut"));
  Double_t XMin = 0.;
  Double_t XMax = 12.;
  //-----------------------------------
  ...
  h0->GetYaxis()->SetRangeUser(1e-4, 10.);
  h0->SetMarkerSize(1);
  ...
  tex->DrawLatex(0.2, 0.8, "0-100%");
  leg->Draw();
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/JetPtUncer/Lambda_Spect_w_JetPtThreshold_0100.pdf"));
  can->SaveAs(Form("./figure/JetPtUncer/Lambda_Spect_w_JetPtThreshold_0100.eps"));
  CanvasEnd(can);
  return;
}
