...
void UncerTot(const TString sType = "Kshort", const TString CentMin = "0", const TString CentMax = "100"){
  ...
  auto hJE2 = (TH1D*)GetTH1D(sPath, sEJetFile, sEJetList, "hFit"); hJE2->SetName("JCErr");
  ...
  leg->Draw();
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/%s_JCUncertCont_%s%s.eps", sType.Data(), CentMin.Data(), CentMax.Data()));
