void PlotRho_JC02_PtJ10(const Bool_t bPrint=kFALSE)
{
...
//=============================================================================
  ...
  TList *list = (TList*)file->Get("listHist_JR02_JC02_PtJ10");
  file->Close();
...
//=============================================================================
  ...
  can = MakeCanvas("Rho_Kshort_JC02_PtJ10"); can->SetLogy();
  ...
  tex->DrawLatex(0.16, 0.84, "Jet: anti-#it{k}_{T}, #it{R}=0.2, #it{p}_{T,jet}^{ch}>10 GeV/#it{c}, |#eta_{jet}|<0.35");
  tex->DrawLatex(0.16, 0.18, "K_{s}^{0}, |#eta|<0.75");
  CanvasEnd(can); if (bPrint) can->Print(Form("%s.pdf",can->GetName()));
//=============================================================================

  can = MakeCanvas("Rho_Lambda_JC02_PtJ10"); can->SetLogy();
  frm = can->DrawFrame(dflx, dfly, dfux, dfuy);
  SetupFrame(frm, dlsx, dlsy, dtsx, dtsy, dtox, dtoy, stnx, stny);
