...
void Ratio_toXM(){
  ...
  TString sLatex(Form("p-Pb at #sqrt{#it{s}_{NN}} = 5.02 TeV"));
  ...
  auto f0= TFile::Open("./published/AnalysisResults_XM.root", "read");
  auto l1 = (TList*)f0->Get(Form("listHD_JC04_JR04_PtJ10"));
  f0->Close();
  ...
  //-----------------------------------
  DrawHisto(h0, cLine[0], sMark[0], "same"); DrawGraph(g0, cLine[0], "E2"); leg->AddEntry(h0, "This Ana");
  DrawHisto(h1, cLine[1], sMark[1], "same"); DrawGraph(g1, cLine[1], "E2"); leg->AddEntry(h1, "Run I");
  ...
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/ThisAna/LKRatio_ComparetToALICE_pPb.eps"));
