...
void PKRatio(){
  ...
  TString sFile[] = {"PtSpect_ThisAna_TestPCL.root", "PtSpect_ThisAna.root"};
  TString sK[] = {"hPtPC_Kshort", "hPtPC_Kshort"};
  TString sP[] = {"hPtPC_Xi", "hPtPC_Xi"};
  TString sA[] = {"hPtPC_Xi", "hPtPC_Xi"};
 
  //TString sCent[] = {"0100", "010", "1040", "40100"};
  //TString sLeg[] = {"0100", "0-10%", "10-40%", "40-100%"};
  TString sCent[] = {"1100", "0100"};
  TString sLeg[] = {"1-100%", "0-100%"};
  //TString sCent[] = {"40100"}; 
  ...
  for (Int_t i = 0; i< 1; ++i){
    for(Int_t j = 0; j< sizeof(sK)/sizeof(TString); ++j){
      ...
      TString sMyHistL = Form("%s_%s", sP[j].Data(), sCent[i].Data()); 
      TString sMyHistA = Form("%s_%s", sA[j].Data(), sCent[i].Data()); 
      TString sMyHistK = Form("%s_%s", sK[j].Data(), sCent[i].Data()); 
  ...
  gStyle->SetOptStat("");
  CanvasEnd(can);
  //can->SaveAs(Form("./figure/ThisAna/XitoKshortRatio_%s_woUncertainty_TestPCL.eps"));
  return;
}
