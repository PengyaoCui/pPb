...
void UncerRatio(const TString sType = "Xi", const TString CentMin = "0", const TString CentMax = "100"){
  
  const TString sK = "Kshort";
  auto sPath = "./result"; 
  auto sFile = "FinalSpect_ThisAna.root";  
  auto sList = Form("%s_%s%s", sType.Data(), CentMin.Data(), CentMax.Data()); 
  auto sKList = Form("%s_%s%s", sK.Data(), CentMin.Data(), CentMax.Data()); 
  ...
  //===========================UESub
  auto hJCK = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtJC_Kshort_%s%s", CentMin.Data(), CentMax.Data())); hJCK->SetName("hJCK");
  auto hNJK = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtNJ_Kshort_%s%s", CentMin.Data(), CentMax.Data())); hNJK->SetName("hNJK");
  auto hOCK = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtOC_Kshort_%s%s", CentMin.Data(), CentMax.Data())); hOCK->SetName("hOCK");
  auto hPCK = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtPC_Kshort_%s%s", CentMin.Data(), CentMax.Data())); hPCK->SetName("hPCK");
  ...
  TH1D* hPC = nullptr;
  if(sType != "Lambda_sum"){
    hJC =  (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtJC_%s_%s%s", sType.Data(), CentMin.Data(), CentMax.Data()));hJC->SetName("hJC");
    hNJ =  (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtNJ_%s_%s%s", sType.Data(), CentMin.Data(), CentMax.Data()));hNJ->SetName("hNJ");
    hOC =  (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtOC_%s_%s%s", sType.Data(), CentMin.Data(), CentMax.Data()));hOC->SetName("hOC");
    hPC =  (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtPC_%s_%s%s", sType.Data(), CentMin.Data(), CentMax.Data()));hNJ->SetName("hPC");
  }
  if(sType == "Lambda_sum"){
    hJC = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtJC_Lambda_%s%s", CentMin.Data(), CentMax.Data())); hJC->SetName("hJC");
    auto hJC2 = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtJC_AntiLa_%s%s", CentMin.Data(), CentMax.Data()));hJC2->SetName("hJC2");  
    hJC->Add(hJC2);hJC->SetName("hJC");

    hNJ = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtNJ_Lambda_%s%s", CentMin.Data(), CentMax.Data())); hNJ->SetName("hNJ");
    auto hNJ2 = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtNJ_AntiLa_%s%s", CentMin.Data(), CentMax.Data()));hNJ2->SetName("hNJ2");  
    hNJ->Add(hNJ2);hNJ->SetName("hNJ");
    hOC = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtOC_Lambda_%s%s", CentMin.Data(), CentMax.Data())); hOC->SetName("hOC");
    auto hOC2 = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtOC_AntiLa_%s%s", CentMin.Data(), CentMax.Data())); hOC2->SetName("hOC2");
    hOC->Add(hOC2);hOC->SetName("hOC");
    
    hPC = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtPC_Lambda_%s%s", CentMin.Data(), CentMax.Data())); hPC->SetName("hPC");
    auto hPC2 = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtPC_AntiLa_%s%s", CentMin.Data(), CentMax.Data()));hPC2->SetName("hPC2");  
  ...
  for(Int_t i = 1; i<= hNJR->GetNbinsX(); i++){
    ...
    heNJR->SetBinContent(i, TMath::Abs(TMath::Abs(dNJR)-TMath::Abs(dJER))/TMath::Sqrt(12.)); 
    heOCR->SetBinContent(i, TMath::Abs(TMath::Abs(dOCR)-TMath::Abs(dJER))/TMath::Sqrt(12.)); 
  }       
  ...
  auto hJPt0K = (TH1D*)GetTH1D(sPath, sFile3, Form("Kshort_JetPt_%s%s", CentMin.Data(), CentMax.Data()), "hist_0"); hJPt0K->SetName("hJPt0K");
  auto hJPt1K = (TH1D*)GetTH1D(sPath, sFile3, Form("Kshort_JetPt_%s%s", CentMin.Data(), CentMax.Data()), "hist_1"); hJPt1K->SetName("hJPt1K");
  TH1D* hJPt0 = nullptr;
  TH1D* hJPt1 = nullptr;
  if(sType != "Lambda_sum"){
    hJPt0 =(TH1D*)GetTH1D(sPath, sFile3, Form("%s_JetPt_%s%s", sType.Data(), CentMin.Data(), CentMax.Data()), "hist_0"); hJPt0->SetName("hJPt0");
    hJPt1 =(TH1D*)GetTH1D(sPath, sFile3, Form("%s_JetPt_%s%s", sType.Data(), CentMin.Data(), CentMax.Data()), "hist_1"); hJPt1->SetName("hJPt1");
  }else{
    hJPt0 =(TH1D*)GetTH1D(sPath, sFile3, Form("Lambda_JetPt_%s%s", CentMin.Data(), CentMax.Data()), "hist_0"); hJPt0->SetName("hJPt0");
    hJPt1 =(TH1D*)GetTH1D(sPath, sFile3, Form("Lambda_JetPt_%s%s", CentMin.Data(), CentMax.Data()), "hist_1"); hJPt1->SetName("hJPt1");
    auto hJPt00 =(TH1D*)GetTH1D(sPath, sFile3, Form("AntiLa_JetPt_%s%s", CentMin.Data(), CentMax.Data()), "hist_0"); hJPt00->SetName("hJPt00");
    auto hJPt11 =(TH1D*)GetTH1D(sPath, sFile3, Form("AntiLa_JetPt_%s%s", CentMin.Data(), CentMax.Data()), "hist_1"); hJPt11->SetName("hJPt11");
  ...
  l->Write(Form("%s_toKRatio_%s%s",sType.Data(), CentMin.Data(), CentMax.Data()), TObject::kSingleKey);
  f->Close();
  ...
  if(sType == "Lambda_sum")tex->DrawLatex(0.15, 0.63, Form("#frac{#Lambda + #bar{#Lambda}}{2K^{0}_{S}} %s-%s%%", CentMin.Data(), CentMax.Data()));
  if(sType == "Xi" || sType == "Omega")tex->DrawLatex(0.15, 0.63, Form("#frac{#%s + #bar{#%s}}{2K^{0}_{S}} %s-%s%%", sType.Data(), sType.Data(), CentMin.Data(), CentMax.Data()));
  leg->Draw();
  gStyle->SetOptStat("");
  //can->SaveAs(Form("./figure/ThisAna/%s_toKRatio_%s%s.eps", sType.Data(), CentMin.Data(), CentMax.Data()));
  ...
  leg1->Draw();
  gStyle->SetOptStat("");
  can1->SaveAs(Form("./figure/CheckUEsubUncer/%s_toK_Uncert_%s%s.pdf", sType.Data(), CentMin.Data(), CentMax.Data()));
  can1->SaveAs(Form("./figure/CheckUEsubUncer/%s_toK_Uncert_%s%s.eps", sType.Data(), CentMin.Data(), CentMax.Data()));
  can1->SaveAs(Form("./figure/CheckUEsubUncer/%s_toK_Uncert_%s%s.png", sType.Data(), CentMin.Data(), CentMax.Data()));
  //DrawAliLogo(0.65, 0.90, 24, kTRUE);
  CanvasEnd(can1);
  return;
}
