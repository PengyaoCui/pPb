...
void SpectRatiowCent(const TString sType = "Kshort"){
  ...
  //leg -> SetNColumns(3);
  auto XMin = 0.; auto XMax = 0.; auto YMin = 0.; auto YMax = 3.5;
  ...
  if(sType == "Kshort"){
      tex->DrawLatex(0.15, 0.7, Form("K^{0}_{S} in jet"));
  }
  if(sType == "Lambda_sum"){
      tex->DrawLatex(0.15, 0.7, Form("#Lambda + #bar{#Lambda} in jet"));
  }
  if(sType == "Xi" || sType == "Omega"){
    tex->DrawLatex(0.15, 0.7, Form("#%s in jet", sType.Data()));
  }
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/ThisAna/JE_%s_SpectRatio_0100.eps", sType.Data()));
  can->SaveAs(Form("./figure/ThisAna/JE_%s_SpectRatio_0100.pdf", sType.Data()));
  can->SaveAs(Form("./figure/ThisAna/JE_%s_SpectRatio_0100.png", sType.Data()));
