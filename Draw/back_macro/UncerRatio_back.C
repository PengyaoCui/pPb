...
void UncerRatio(const TString CentMin = "0", const TString CentMax = "100"){
  ...
  TString sJH0 = "Loose"; TString sJH1 = "Tight";
  ...
  auto hEILKRAbs = (TH1D*)hEILKR->Clone("hEILKRAbs");
  auto hEIXKRAbs = (TH1D*)hEIXKR->Clone("hEIXKRAbs");
  Double_t dEILK[nI];
  Double_t dEIXK[nI];
  for(Int_t i = 1; i<=nI; i++){
    ...
    hEILKRAbs->SetBinContent(i, dEILKR); 
    ...
    hEIXKRAbs->SetBinContent(i, dEIXKR); 
  }

  //
  //JC Strange rec 
  //
  auto hEIJLKR = (TH1D*)hEILKRAbs->Clone("hEIJLKR");
  DeNormHistBinWidth(hEIJLKR); 
  hEJCILKR = RebinTH1D(hEIJLKR, hEJCILKR);
  NormHistBinWidth(hEJCILKR); 
  hEJCILKR->SetName("hEJCILKR");

  auto hEIJXKR = (TH1D*)hEIXKRAbs->Clone("hEIJXKR");
  ...
  hEJCJLKR0->Add(hJL0, hJA0); hEJCJLKR0->Divide(hEJCJLKR0, hJK0, 1., 2.);
  hEJCJLKR1->Add(hJL1, hJA1); hEJCJLKR1->Divide(hEJCJLKR1, hJK1, 1., 2.);
  ...
  for(Int_t i = 1; i<=nJ; i++){
    ...
    hEJCJLKR->SetBinContent(i, dEJLKR/(dJ*TMath::Sqrt(12.)));
  }
  
  auto hEJCJXKR0=(TH1D*)hEJCJXKR->Clone("hEJCJXKR0"); hEJCJXKR0->Reset();
  auto hEJCJXKR1=(TH1D*)hEJCJXKR->Clone("hEJCJXKR1"); hEJCJXKR1->Reset();
  hEJCJXKR0->Divide(hJX0, hJK0, 1., 2.);
  hEJCJXKR1->Divide(hJX1, hJK1, 1., 2.);
  hCJXKR->Divide(hCJX, hCJK, 1., 2.); hCJXKR->SetName("hCJXKR");

  for(Int_t i = 1; i<=nJ; i++){
    auto dJ0 = hEJCJXKR0->GetBinContent(i);
    auto dJ1 = hEJCJXKR1->GetBinContent(i);
    auto dJ = hCJXKR->GetBinContent(i);
    auto dEJXKR = TMath::Max(TMath::Abs(dJ0-dJ), TMath::Abs(dJ1-dJ));
    hEJCJXKR->SetBinContent(i, dEJXKR/(dJ*TMath::Sqrt(12.)));
  }
  ...
  for(Int_t i = 1; i<=nJ; i++){
    ...
    auto dEULKR = TMath::Abs(dU-dO);
    hEJCULKR->SetBinContent(i, dEULKR/(TMath::Sqrt(12.)*dJ));
  }
  hEJCULKR->SetBinContent(nJ, 0.);
  TH1D *hSumLK[] = {hEJCILKR, hEJCJLKR, hEJCULKR};
  hEJLKR = (TH1D*)QuadraticSum(3, hSumLK);hEJLKR->SetName("hEJLKR"); 
  
  //====================================== 
  hCUXKR->Divide(hCUX, hCUK, 1., 2.); hCUXKR->SetName("hCUXKR");
  auto hCNXKR=(TH1D*)hCUXKR->Clone("hCNXKR"); hCNXKR->Reset();
  hCNXKR->Divide(hCNX, hCNK, 1., 2.); hCNXKR->SetName("hCNXKR");
  auto hCOXKR=(TH1D*)hCUXKR->Clone("hCOXKR"); hCOXKR->Reset();
  hCOXKR->Divide(hCOX, hCOK, 1., 2.); hCOXKR->SetName("hCOXKR");
  
  for(Int_t i = 1; i<=nJ; i++){
    auto dO = hCOXKR->GetBinContent(i); 
    auto dN = hCNXKR->GetBinContent(i); 
    auto dU = hCUXKR->GetBinContent(i);
    auto dJ = hCJXKR->GetBinContent(i); 
    //auto dEUXKR = TMath::Max(TMath::Abs(dO-dU), TMath::Abs(dN-dU));
    auto dEUXKR = TMath::Abs(dO-dU);
    hEJCUXKR->SetBinContent(i, dEUXKR/(TMath::Sqrt(12.)*dJ));
  }
  hEJCUXKR->SetBinContent(nJ, 0.);
  TH1D *hSumXK[] = {hEJCIXKR, hEJCJXKR, hEJCUXKR};
  hEJXKR = (TH1D*)QuadraticSum(3, hSumXK);hEJXKR->SetName("hEJXKR"); 
  ...
  auto hEIUXKR = (TH1D*)hEIXKR->Clone("hEIUXKR");
  DeNormHistBinWidth(hEIUXKR); 
  hEUXKR = RebinTH1D(hEIUXKR, hEUXKR);
  NormHistBinWidth(hEUXKR); 
  hEUXKR->SetName("hEUXKR");
  //for(Int_t i = 1; i<=nJ; i++){
  //  auto dCU = hCUXKR->GetBinContent(i);
  //  auto dEU = hEUXKR->GetBinContent(i);
  //  hEUXKR->SetBinContent(i, dEU/dCU);
  //}
  ...
  lLK->Add(hEJCJLKR);
  lLK->Add(hEJCULKR);
  lLK->Write(Form("LKR_Uncer_%s%s", CentMin.Data(), CentMax.Data()), TObject::kSingleKey);
  ...
  lXK->Add(hEJCJXKR);
  lXK->Add(hEJCUXKR);
  lXK->Write(Form("XKR_Uncer_%s%s", CentMin.Data(), CentMax.Data()), TObject::kSingleKey);
  f->Close();
  ...
  DrawHisto(hEJXKR, cLine[0], sMark[0], "same"); leg1->AddEntry(hEJXKR, "Total","l");
  hEJXKR->GetYaxis()->SetTitle("Relative systematic error");
  hEJXKR->GetYaxis()->SetRangeUser(0, 1);
  DrawHisto(hEJCIXKR, cLine[1], sMark[1], "same");leg1->AddEntry(hEJCIXKR, "Strange reconstruction","l");
  DrawHisto(hEJCJXKR, cLine[2], sMark[2], "same");leg1->AddEntry(hEJCJXKR, "Jet #it{p}_{T} scale","l");
  DrawHisto(hEJCUXKR, cLine[3], sMark[3], "same");leg1->AddEntry(hEJCUXKR, "UE subtraction","l");
  //-----------------------------------
  tex1 =  new TLatex();
  tex1->SetNDC();
  ...
  tex1->DrawLatex(0.15, 0.63, Form("#frac{#Xi + #bar{#Xi}}{2K^{0}_{S}} %s-%s%%", CentMin.Data(), CentMax.Data()));
  leg1->Draw();
  gStyle->SetOptStat("");
  //can1->SaveAs(Form("./figure/XKR_JCUncertCont_%s%s.eps", CentMin.Data(), CentMax.Data()));
  //DrawAliLogo(0.65, 0.90, 24, kTRUE);
  CanvasEnd(can1);
