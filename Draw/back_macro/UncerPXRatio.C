...
void UncerPXRatio(const TString sType = "Omega", const TString CentMin = "0", const TString CentMax = "100"){
  ...
  //===========================UESub
  auto hNJK = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtNJ_Xi_%s%s", CentMin.Data(), CentMax.Data())); hNJK->SetName("hNJK");
  auto hOCK = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtOC_Xi_%s%s", CentMin.Data(), CentMax.Data())); hOCK->SetName("hOCK");
  auto hPCK = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtPC_Xi_%s%s", CentMin.Data(), CentMax.Data())); hPCK->SetName("hPCK");
  ...
  auto hJPt0K = (TH1D*)GetTH1D(sPath, sFile3, Form("Xi_JetPt_%s%s", CentMin.Data(), CentMax.Data()), "hist_0"); hJPt0K->SetName("hJPt0K");
  auto hJPt1K = (TH1D*)GetTH1D(sPath, sFile3, Form("Xi_JetPt_%s%s", CentMin.Data(), CentMax.Data()), "hist_1"); hJPt1K->SetName("hJPt1K");
  ...
  l->Write(Form("%s_toXRatio_%s%s",sType.Data(), CentMin.Data(), CentMax.Data()), TObject::kSingleKey);
  f->Close();
  ...
  tex->DrawLatex(0.15, 0.63, Form("#frac{#%s + #bar{#%s}}{#Xi + #bar{#Xi}} %s-%s%%", sType.Data(), sType.Data(), CentMin.Data(), CentMax.Data()));
  leg->Draw();
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/ThisAna/%s_toXRatio_%s%s.eps", sType.Data(), CentMin.Data(), CentMax.Data()));
  ...
  leg1->Draw();
  gStyle->SetOptStat("");
  can1->SaveAs(Form("./figure/Uncert/%s_toX_Uncert_%s%s.eps", sType.Data(), CentMin.Data(), CentMax.Data()));
  //DrawAliLogo(0.65, 0.90, 24, kTRUE);
  CanvasEnd(can1);
  return;
}
