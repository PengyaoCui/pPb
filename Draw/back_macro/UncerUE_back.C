...
void UncerUE(const TString sType = "Xi", const TString CentMin = "0", const TString CentMax = "100"){
  
  TString sPath = "/home/cuipengyao/study/pPb/Local/ThisAna/JEBins"; 
  ...
  TString OCList = Form("%s_%s_Default_OC_%s%s", sList.Data(), sType.Data(), CentMin.Data(), CentMax.Data());
  TString PCLList = Form("%s_%s_Default_PCL_%s%s", sList.Data(), sType.Data(), CentMin.Data(), CentMax.Data());
  TString PCUList = Form("%s_%s_Default_PCU_%s%s", sList.Data(), sType.Data(), CentMin.Data(), CentMax.Data());
  TString NJList = Form("%s_%s_Default_NJ_%s%s", sList.Data(), sType.Data(), CentMin.Data(), CentMax.Data());
  ...
  for(Int_t i = 1; i<=h1->GetNbinsX(); i++){
    ...
    h2->SetBinContent(i, TMath::Abs(V3-V1)/TMath::Sqrt(12.)); 
    h2->SetBinError(i, TMath::Sqrt(E3*E3 + E1*E1)/TMath::Sqrt(12.)); 
    h3->SetBinContent(i, TMath::Abs(V4-V1)/TMath::Sqrt(12.)); 
    h3->SetBinError(i, TMath::Sqrt(E4*E4 + E1*E1)/TMath::Sqrt(12.)); 
  ...
  TList *l = new TList();
  l->Add(hUMax);
  l->Write(Form("%s_UESub_%s%s",sType.Data(), CentMin.Data(), CentMax.Data()), TObject::kSingleKey);
  f->Close();
  ...
  leg->Draw();
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/%s_UESub_%s%s.eps", sType.Data(), CentMin.Data(), CentMax.Data()));
  CanvasEnd(can);
  return;
}
