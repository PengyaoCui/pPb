...
//TH1D * RebinTH1D(TH1D const *hF, TH1D const *hR)//Rebin hF to hR 
...
TH1* GetTH1FromFile(const TString sPath,
  ...
  TString sFile = Form("%s/%s",sPath.Data(),sFile.Data());
...
TH1* GetTH1FromFile(const TString sPath,
  ...
  TString sFile = Form("%s/%s",sPath.Data(),sFile.Data());
  TFile*   file = TFile::Open(sFile,"read"); if (!file->IsOpen()) return 0x0;
...
TH1D* NormIncl(TH1D* hIncl, TH1D* hEvent){
  ...
  for (Int_t k=1; k<= hIncl->GetNbinsX(); k++){
    ...
    //Double_t dNormIncl = dIncl/(dEvent*2*0.45*TMath::TwoPi());
    //Double_t  dNormErr = dInclErr/(dEvent*2*0.45*TMath::TwoPi());
    Double_t dNormIncl = dIncl/(dEvent*2*0.75*TMath::TwoPi());
    Double_t  dNormErr = dInclErr/(dEvent*2*0.75*TMath::TwoPi());
...
TH1D* NormJC(TH1D* hJC, TH1D* hEvent, Double_t R = 0.4){
  ...
  for (Int_t k=1; k<= hJC->GetNbinsX(); k++){
    ...
    //Double_t  dNormJC = dJC/(dJetEvent*TMath::Pi()*R*R);
    //Double_t  dNormErr = dJCErr/(dJetEvent*TMath::Pi()*R*R);
...
//____________________________________________________________________
TH1D* NormPC(TH1D* hPC, TH1D* hEvent, Double_t R = 0.4){
  if(!(hPC && hEvent)) return;
  Double_t dJetEvent = hEvent->GetBinContent(4);
  cout<<"..............."<<"Jet Events = "<<dJetEvent<<"................"<<endl;

  //hJC->Scale(1/(dJetEvent*TMath::Pi()*R*R));
  for (Int_t k=1; k<= hPC->GetNbinsX(); k++){
    Double_t      dPC = hPC->GetBinContent(k);
    Double_t   dPCErr = hPC->GetBinError(k);
    //Double_t  dNormJC = dJC/(dJetEvent*TMath::Pi()*R*R);
    //Double_t  dNormErr = dJCErr/(dJetEvent*TMath::Pi()*R*R);
    Double_t  dNormPC = dPC/(dJetEvent*2*0.75*TMath::TwoPi()*4*0.06);
    Double_t  dNormErr = dPCErr/(dJetEvent*2*0.75*TMath::TwoPi()*4*0.06);
    hPC->SetBinContent(k, dNormPC);
    hPC->SetBinError(k, dNormErr); 
  }
...
TH1D* NormPC(TH1D* hPCL, TH1D* hPCU, TH1D* hEvent, Double_t R = 0.4){
  ...
  for (Int_t k=1; k<= hPCL->GetNbinsX(); k++){
    ...
    //Double_t  dNormPC = dPC/(dJetEvent*4*TMath::Pi()*R*R);
    //Double_t dNormErr = dPCErr/(dJetEvent*4*TMath::Pi()*R*R);
    Double_t  dNormPC = dPC/(dJetEvent*2*0.75*TMath::TwoPi()*4*0.06);
    Double_t dNormErr = dPCErr/(dJetEvent*2*0.75*TMath::TwoPi()*4*0.06);
...
TH1D* NormOC(TH1D* hOC, TH1D* hEvent, Double_t R = 0.4){
  ...
  for (Int_t k=1; k<= hOC->GetNbinsX(); k++){
    ...
    //Double_t  dNormOC = dOC/(dJetEvent*(2*0.75*TMath::TwoPi()-TMath::Pi()*R*R));
    //Double_t  dNormErr = dOCErr/(dJetEvent*(2*0.75*TMath::TwoPi()-TMath::Pi()*R*R));
...
TH1D* NormNJ(TH1D* hNJ, TH1D* hEvent){
  ...
  Double_t dEvent = hEvent->GetBinContent(2);//Xaxis range is -10 to 200
...
TH1D* Subtract(TH1D* h1, TH1D* h2){
  ...
  for(Int_t k=1; k<= h1->GetNbinsX(); k++){
    ...
    Double_t    dh = dh1 - dh2;
    Double_t  dErr = TMath::Sqrt(dErr1*dErr1+dErr2*dErr2);
