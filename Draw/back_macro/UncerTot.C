...
void UncerTot(const TString sType = "Lambda", const TString CentMin = "0", const TString CentMax = "100"){
  ...
  auto sEInclHist = Form("%s_InclUncer_%s%s", sType.Data(), CentMin.Data(), CentMax.Data());
  ...
  //l->Add(hJErrContJ);
  //l->Add(hJErrContU);
  //l->Write(Form("%s_UncerTot_%s%s",sType.Data(), CentMin.Data(), CentMax.Data()), TObject::kSingleKey);
  //f->Close();
  ...
  //if(sType == "Xi" || sType == "Lambda")hJErr->GetYaxis()->SetRangeUser(0, 0.8);
  ...
  tex->DrawLatex(0.15, 0.65, Form("%s %s-%s%%", sType.Data(), CentMin.Data(), CentMax.Data()));
  leg->Draw();
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/CheckUEsubUncer/%s_JCUncertCont_%s%s_MaxMin.eps", sType.Data(), CentMin.Data(), CentMax.Data()));
  can->SaveAs(Form("./figure/CheckUEsubUncer/%s_JCUncertCont_%s%s_MaxMin.pdf", sType.Data(), CentMin.Data(), CentMax.Data()));
  can->SaveAs(Form("./figure/CheckUEsubUncer/%s_JCUncertCont_%s%s_MaxMin.png", sType.Data(), CentMin.Data(), CentMax.Data()));
