...
void XLRatio(){
  ...
  TString sFile = "PtSpect_Eta_V0BinCascade.root";
  TString sList = "";
  ...
  TString sCent[] = { "40100"};//, "1040", "40100"};
  ...
  for (Int_t i = 0; i< nHist; ++i){
    for(Int_t j = 0; j< sizeof(sX)/sizeof(TString); ++j){
      ...
      TString sMyHistL = Form("%s_%s", sL[j].Data(), sCent[i].Data()); 
      TString sMyHistA = Form("%s_%s", sA[j].Data(), sCent[i].Data()); 
      TString sMyHistX = Form("%s_%s", sX[j].Data(), sCent[i].Data()); 
      ...
      hL = (TH1D*)GetTH1D(sMyPath.Data(), "PtSpect_Eta.root", sMyList.Data(), sMyHistL.Data());
      hL->SetName(Form("Lambda_%d%d", i, j)); 
      //DeNormHistBinWidth(hL); 
      //hL=RebinTH1D(hL, hX);
      
      hA = (TH1D*)GetTH1D(sMyPath.Data(), "PtSpect_Eta.root", sMyList.Data(), sMyHistA.Data());
      ...
      //f->cd(); hRatio->Write(Form("XLRatio_%s", sCent[i].Data()), TObject::kSingleKey);
