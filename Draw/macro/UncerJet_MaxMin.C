...
void UncerJet_MaxMin(const TString sType = "Kshort", const TString CentMin = "0", const TString CentMax = "100"){
  ...
  TH1D* hRatio = nullptr;
  TH1D* hU[nhist+1];
  h2 = (TH1D*)GetTH1D(Form("/home/cuipengyao/study/pPb/Local/ThisAna/JE"), sFile.Data(), List.Data(), sHist.Data());
  h2->SetName("Default");
  h2->SetTitle("");
  NormHistBinWidth(h2);  
  ...
  leg->Draw();
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/CheckUEsubUncer/%s_JetPt_%s%s.eps", sType.Data(), CentMin.Data(), CentMax.Data()));
  can->SaveAs(Form("./figure/CheckUEsubUncer/%s_JetPt_%s%s.pdf", sType.Data(), CentMin.Data(), CentMax.Data()));
  can->SaveAs(Form("./figure/CheckUEsubUncer/%s_JetPt_%s%s.png", sType.Data(), CentMin.Data(), CentMax.Data()));
  CanvasEnd(can);
  return;
}
