...
//_____________________________________________________________________________
TH1D* ConvRelErrors(TH1D *hVar, TH1D *hErr)
{

  TH1D *hOut = (TH1D*)hVar->Clone(Form("hOut_%s_%s",hVar->GetName(),hErr->GetName()));
  hOut->Reset();

  for (Int_t k=1; k<=hVar->GetNbinsX(); k++) {
    hOut->SetBinContent(k, hVar->GetBinContent(k));
    hOut->SetBinError(k, hVar->GetBinContent(k)*hErr->GetBinContent(k));
  }
