void Fig3b_pPb(const Bool_t bPrint=kFALSE)
{
...
//=============================================================================
  ...
  TH1D *hHD = (TH1D*)list->FindObject("hLambda_Syst");
  hHD->SetName("HD");  // Total
//=============================================================================

  file = TFile::Open("data/pPb/AnalysisOutputs_SystJE.root", "READ");
  list = (TList*)file->Get("list_SystJE_Lambda_PtJ10");
  file->Close();

  TH1D *hJE = (TH1D*)list->FindObject("hLambdaErr_JC04");
  hJE->SetName("JE");  // Jet #it{p}_{T} scale
//=============================================================================

  file = TFile::Open("data/pPb/AnalysisOutputs_SystUE.root", "READ");
  list = (TList*)file->Get("list_SystUE_Lambda_PtJ10");
  file->Close();

  TH1D *hUE = (TH1D*)list->FindObject("hLambdaErr_JC04");
...
//=============================================================================
  ...
  const Float_t dfly =  0.0, dfuy = 0.9;
