void PlotL2K_VJ_Comp(const UInt_t kAct = 0,
                     const UInt_t kPtj = 0,
                     const UInt_t kPtV = 0)
{
...
//=============================================================================
  ...
  DrawGraph(gK04Syst, wcl[5], "E2"); DrawGraph(gK04Stat, wcl[5], wmk[0], "P");
  ...
  tex->DrawLatex(0.30, 0.90, Form("%s, p-Pb #sqrt{s_{NN}}=5.02 TeV, %s", sVarL[kKshort].Data(), sActL[kAct].Data()));
  tex->DrawLatex(0.30, 0.82, Form("%s, |#eta_{jet}|<0.35, anti-#it{k}_{T}", sPtjL[kPtj].Data()));
  tex->DrawLatex(0.30, 0.74, Form("%s, |#eta_{V^{0}}|<0.75", sPtvL.Data()));
  CanvasEnd(can);
//=============================================================================

  can = MakeCanvas(Form("Lambda_%s", sCanN.Data()));
  can->SetGridx(); can->SetGridy();
  frm = can->DrawFrame(dflx, dfly, dfux, 0.1);
  SetupFrame(frm, dlsx, dlsy, dtsx, dtsy, dtox, dtoy, stnx, stny);

  DrawGraph(gL02Syst, wcl[3], "E2"); DrawGraph(gL02Stat, wcl[3], wmk[0], "P");
  DrawGraph(gL04Syst, wcl[5], "E2"); DrawGraph(gL04Stat, wcl[5], wmk[0], "P");
  ...
  tex->SetNDC();
  tex->SetTextSizePixels(24);
  tex->DrawLatex(0.16, 0.90, Form("%s, p-Pb #sqrt{s_{NN}}=5.02 TeV, %s", sVarL[kLambda].Data(), sActL[kAct].Data()));
...
//=============================================================================
  ...
  can = MakeCanvas(Form("AntiLa_%s", sCanN.Data()));
  can->SetGridx(); can->SetGridy();
  frm = can->DrawFrame(dflx, dfly, dfux, 0.1);
  SetupFrame(frm, dlsx, dlsy, dtsx, dtsy, dtox, dtoy, stnx, stny);

  DrawGraph(gA02Syst, wcl[3], "E2"); DrawGraph(gA02Stat, wcl[3], wmk[0], "P");
  DrawGraph(gA04Syst, wcl[5], "E2"); DrawGraph(gA04Stat, wcl[5], wmk[0], "P");
  DrawGraph(gA03Syst, wcl[1], "E2"); DrawHisto(hA03Stat, wcl[1], wmk[0], "SAME");

  leg = new TLegend(0.70, 0.72, 0.96, 0.96); SetupLegend(leg);
  leg->AddEntry(gA02Stat, sRjeL[kJC02], "LP")->SetTextSizePixels(24);
  leg->AddEntry(hA03Stat, sRjeL[kJC03], "LP")->SetTextSizePixels(24);
  leg->AddEntry(gA04Stat, sRjeL[kJC04], "LP")->SetTextSizePixels(24);
  leg->Draw();
  ...
  tex->SetNDC();
  tex->SetTextSizePixels(24);
  tex->DrawLatex(0.16, 0.90, Form("%s, p-Pb #sqrt{s_{NN}}=5.02 TeV, %s", sVarL[kAntiLa].Data(), sActL[kAct].Data()));
  tex->DrawLatex(0.16, 0.82, Form("%s, |#eta_{jet}|<0.35, anti-#it{k}_{T}", sPtjL[kPtj].Data()));
  tex->DrawLatex(0.16, 0.74, Form("%s, |#eta_{V^{0}}|<0.75", sPtvL.Data()));
  CanvasEnd(can);
//=============================================================================
  ...
  DrawGraph(gR04Syst, wcl[5], "E2"); DrawGraph(gR04Stat, wcl[5], wmk[0], "P");
