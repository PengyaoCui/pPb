...
void FinalSpect(const TString sType = "Kshort", const TString CentMin = "0", const TString CentMax = "100", const Bool_t IsSum = kFALSE){
  ...
  auto sCJCHist = Form("hPtJC_%s_%s%s", sType.Data(), CentMin.Data(), CentMax.Data());
  ...
  auto sCOCHist = Form("hPtOC_%s_%s%s", sType.Data(), CentMin.Data(), CentMax.Data());
  auto sCNJHist = Form("hPtNJ_%s_%s%s", sType.Data(), CentMin.Data(), CentMax.Data());
  ...
  if(sType == "Lambda" && IsSum){
    ...
    auto hAOC1   = (TH1D*)GetTH1D(sPath, sCFile, "", Form("hPtOC_AntiLa_%s%s", CentMin.Data(), CentMax.Data())); hAOC1->SetName("AOCCen");
    auto hANJ1   = (TH1D*)GetTH1D(sPath, sCFile, "", Form("hPtNJ_AntiLa_%s%s", CentMin.Data(), CentMax.Data())); hANJ1->SetName("ANJCen");
