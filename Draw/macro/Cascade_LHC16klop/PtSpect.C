...
void PtSpect(){
  ...
  TString sSaveName = "PtSpect_Xi";
  //TString sTex = "#Xi";
  TString sTex = "Jet: Anti-#it{k}_{T}, #it{R} = 0.4"; 
...
//-------------------------------------------------------------------
  ...
  //-------------------------------------------------------------------
    ...
    if (IsAnaMC){
      ...
      SetAxis(hPtMC, YMin, YMax, XMin, XMax, "d#it{#rho}/d#it{p}_{T} (#it{c}/GeV)");
    ...
    if (IsAnaIncl){
      ...
      SetAxis(hPtIncl, YMin, YMax, XMin, XMax, "d#it{#rho}/d#it{p}_{T} (#it{c}/GeV)");
      if(IsSave) {file->cd(); hPtIncl->Write(Form("hPtIncl_%s", sOpt.Data()), TObject::kSingleKey);}
      for(Int_t i =1; i<=7; i++){hPtIncl->SetBinContent(i, 0); hPtIncl->SetBinError(i, 0);}
      hPtIncl->Draw("same");
      leg->AddEntry(hPtIncl,"Inclusive #Xi","lp");
      ...
      if (IsUncer){
        ...
        Double_t ex[n] = {0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05};
    ...
    if (IsAnaJC){
     ...
     //Sys Uncertainty======>
      if (IsUncer){
        ...
        Double_t ex[n] = {0.05, 0.05, 0.05, 0.05, 0.05, 0.05};
    ...
    if (IsAnaPC){
      ...
      hPtPC->Draw("same");
      leg->AddEntry(hPtPC,"#Xi in UE","lp");
...
//
    ...
    if (IsAnaJE){
      ...
      hPtJE->Draw("same");
      leg->AddEntry(hPtJE,"#Xi in JE","lp");
     //Sys Uncertainty======>
      if (IsUncer){
        const Int_t n = 6;
        Double_t x[n] = {2.2, 2.6, 3.05, 3.65, 4.5, 5.75};
        ...
        Double_t ey3[n] = {0.000276884, 0.000195581, 0.000140336, 9.05768e-05, 4.74774e-05, 9.45003e-06};
        Double_t ey1[n] = {0.042832*y[0], 0.0480318*y[1], 0.0560362*y[2], 0.0641628*y[3], 0.0642275*y[4], 0.0771022*y[5]};
        Double_t ey2[n] = {0.0591979*y[0], 0.0553908*y[1], 0.0288977*y[2], 0.0813561*y[3], 0.0241927*y[4], 0.0459242*y[5]};
    ...
    if (IsAnaOC){
      ...
      hPtOC->Draw("same");
      leg->AddEntry(hPtOC,"#Xi in OC","lp");
    }
    ...
    if (sListNJ.IsNull() || sEffNJ.IsNull()) return;
      ...
      for(Int_t m = 1; m<= hPtOC->GetNbinsX(); m++){ 
        ...
        Double_t dPC = hPtPC->GetBinContent(m);
        Double_t dNJ = hPtNJ->GetBinContent(m);

        if(TMath::Abs(dOC-dPC) >=TMath::Abs(dNJ-dPC)){
          cout<<TMath::Sqrt(1./12.)*TMath::Abs(dPC - dOC)<<endl;
        }else{
          cout<<TMath::Sqrt(1./12.)*TMath::Abs(dPC - dNJ)<<endl;
        }
      //cout<<TMath::Abs(TMath::Sqrt(1./12.)*0.5*(hPtOC->GetBinContent(m) - hPtNJ->GetBinContent(m)))<<endl;
      }
      leg->AddEntry(hPtNJ,"#Xi in NJ","lp");
    }
  }
  ...
  tex->DrawLatex(0.65, 0.91, "ALICE Preliminary");
  tex->SetTextFont(42);
  tex->DrawLatex(0.17, 0.91, "ALICE pp #sqrt{s} = 13 TeV");
  tex->DrawLatex(0.17, 0.83, sTex);
  tex->DrawLatex(0.17, 0.75, "#it{p}_{T, jet}^{ch} > 10 GeV/#it{c}, |#it{#eta}_{jet}| < 0.35");
  tex->DrawLatex(0.17, 0.3, "#Delta#it{R}(#Xi, jet) < 0.4");
  tex->DrawLatex(0.17, 0.2, "|#it{#eta}_{#Xi}| < 0.75");
  leg->Draw();
