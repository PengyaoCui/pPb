...
void ratio_Sum(){
  ...
  for(Int_t i = 1; i<=hSumNu->GetNbinsX(); i++){
    ...
    Double_t dErrNu = 0.;
    for (Int_t j = 1; j<= i; j++ ){
      dErrNu += hNumerator->GetBinError(j) * hNumerator->GetBinError(j);
    }

    //hSumNu->SetBinContent(i, hNumerator->Integral(i, hSumNu->GetNbinsX()));
    //Double_t dErrNu = 0.;
    //for (Int_t j = i; j<= hSumNu->GetNbinsX(); j++ ){ 
    //  dErrNu += hNumerator->GetBinError(j) * hNumerator->GetBinError(j);
    //}

    hSumNu->SetBinError(i, TMath::Sqrt(dErrNu));
  }
  //hSumNu->Draw("same");
  SetLine(hSumNu,cLine[0], sMark[0]);
  SetAxis(hSumNu, YMin, YMax, XMin, XMax, "Probability",sCuts);
  ...
  for(Int_t i = 1; i<=hSumNu0->GetNbinsX(); i++){
    hSumNu0->SetBinContent(i, hNumerator0->Integral(1, i));
    Double_t dErrNu0 = 0.;
    for (Int_t j = 1; j<= i; j++ ){
      dErrNu0 += hNumerator0->GetBinError(j) * hNumerator0->GetBinError(j);
    }
    //hSumNu0->SetBinContent(i, hNumerator0->Integral(i, hSumNu0->GetNbinsX()));
    //Double_t dErrNu0 = 0.;
    //for (Int_t j = i; j<= hSumNu->GetNbinsX(); j++ ){ 
    //  dErrNu0 += hNumerator0->GetBinError(j) * hNumerator0->GetBinError(j);
    //}
    hSumNu0->SetBinError(i, TMath::Sqrt(dErrNu0));
  }
  ...
  for(Int_t i = 1; i<=hSumDe->GetNbinsX(); i++){
    hSumDe->SetBinContent(i, hDenominator->Integral(1, i));
    Double_t dErrDe = 0.;
    for (Int_t j = 1; j<= i; j++ ){
      dErrDe += hDenominator->GetBinError(j) * hDenominator->GetBinError(j);
    }
    //hSumDe->SetBinContent(i, hDenominator->Integral(i, hSumDe->GetNbinsX()));
    //Double_t dErrDe = 0.;
    //for (Int_t j = i; j<= hSumNu->GetNbinsX(); j++ ){ 
    //  dErrDe += hDenominator->GetBinError(j) * hDenominator->GetBinError(j);
    //}
    hSumDe->SetBinError(i, TMath::Sqrt(dErrDe));
  }
