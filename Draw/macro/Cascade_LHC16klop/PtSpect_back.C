...
void PtSpect(){
  ...
  TString sSaveName = "PtSpect_Xi_wUncer";
  TString sTex = "#Xi";
...
//
    ...
    if (IsAnaJE){
     ...
     //Sys Uncertainty======>
      if (IsUncer){
        ...
        Double_t ey[n] = {0.000580924, 0.000447177, 0.0002867, 0.000232897, 7.56544e-05, 2.64858e-05};
    ...
    if (sListNJ.IsNull() || sEffNJ.IsNull()) return;
      ...
      hPtNJ->Draw("same");
      //for(Int_t m = 1; m<= hPtOC->GetNbinsX(); m++){ cout<<TMath::Abs(TMath::Sqrt(1./12.)*(hPtOC->GetBinContent(m) - hPtNJ->GetBinContent(m)))<<endl;}
