...
void PtSpect_LargeBinWidth(){
  TString   sType[] = {"Xi"};//"XiPlus"; "XiMinus"
  //TString sSaveName = "PtSpect_JC_test_everycut";
  TString sSaveName = "PtSpect_LHC16khl_LargeBinWidth";
  TString sTex = "Period : LHC16k LHC16h and LHC16l";
  ...
  Bool_t   IsNorm(1);
//===================================================================
  TString sEventFilePath = "/home/cuipengyao/study/code/Cascade_LHC16khl_total/JC_LargeBinWidth";
  ...
  //-------------------------------------------------------------------
    ...
    if (IsAnaJC){
      ...
      if (IsEffCorr) {
        ...
        //TH1D* hEffJCk = (TH1D*)GetTH1FromFile(sEventFilePathk, sEffJetFile, sEffJC, sEff); 
        //hEffJCk->SetName(Form("hEffJCk_%s", sOpt.Data()));
        //hEffJCk->Scale(2.49632e8/(2.49632e8 + 1.28757e8 + 6.72309e7));

        //TH1D* hEffJCh = (TH1D*)GetTH1FromFile(sEventFilePathh, sEffJetFile, sEffJC, sEff); 
        //hEffJCh->SetName(Form("hEffJCh_%s", sOpt.Data()));
        //hEffJCh->Scale(1.28757e8/(2.49632e8 + 1.28757e8 + 6.72309e7));

        //TH1D* hEffJCl = (TH1D*)GetTH1FromFile(sEventFilePathl, sEffJetFile, sEffJC, sEff); 
        //hEffJCl->SetName(Form("hEffJCl_%s", sOpt.Data()));
        //hEffJCl->Scale(6.72309e7/(2.49632e8 + 1.28757e8 + 6.72309e7));

        //TH1D *hEffJC = (TH1D*) hEffJCk->Clone(Form("hEffJC_%s", sOpt.Data()));
        //hEffJC->Add(hEffJCh);
        //hEffJC->Add(hEffJCl);
        //hPtJC = RatioTH1D(hPtJC, hEffJC);
      }
    ...
    if (IsAnaPC){
      ...
      if (IsEffCorr){
        ...
        //TH1D* hEffPCLk = (TH1D*)GetTH1FromFile(sEventFilePathk, sEffJetFile, sEffPCL, sEff);
        //hEffPCLk->SetName(Form("hEffPCLk_%s", sOpt.Data()));
        //hEffPCLk->Scale(2.49632e8/(2.49632e8 + 1.28757e8 + 6.72309e7));

        //TH1D* hEffPCLh = (TH1D*)GetTH1FromFile(sEventFilePathh, sEffJetFile, sEffPCL, sEff);
        //hEffPCLh->SetName(Form("hEffPCLh_%s", sOpt.Data()));
        //hEffPCLh->Scale(1.28757e8/(2.49632e8 + 1.28757e8 + 6.72309e7));

        //TH1D* hEffPCLl = (TH1D*)GetTH1FromFile(sEventFilePathl, sEffJetFile, sEffPCL, sEff);
        //hEffPCLl->SetName(Form("hEffPCLl_%s", sOpt.Data()));
        //hEffPCLl->Scale(6.72309e7/(2.49632e8 + 1.28757e8 + 6.72309e7));

        //TH1D *hEffPCL = (TH1D*) hEffPCLk->Clone(Form("hEffPCL_%s", sOpt.Data()));
        //hEffPCL->Add(hEffPCLh);
        //hEffPCL->Add(hEffPCLl);
      ...
      if (IsEffCorr){
        ...
        //TH1D* hEffPCUk = (TH1D*)GetTH1FromFile(sEventFilePathk, sEffJetFile, sEffPCU, sEff);
        //hEffPCUk->SetName(Form("hEffPCUk_%s", sOpt.Data()));
        //hEffPCUk->Scale(2.49632e8/(2.49632e8 + 1.28757e8 + 6.72309e7));

        //TH1D* hEffPCUh = (TH1D*)GetTH1FromFile(sEventFilePathh, sEffJetFile, sEffPCU, sEff);
        //hEffPCUh->SetName(Form("hEffPCUh_%s", sOpt.Data()));
        //hEffPCUh->Scale(1.28757e8/(2.49632e8 + 1.28757e8 + 6.72309e7));

        //TH1D* hEffPCUl = (TH1D*)GetTH1FromFile(sEventFilePathl, sEffJetFile, sEffPCU, sEff);
        //hEffPCUl->SetName(Form("hEffPCUl_%s", sOpt.Data()));
        //hEffPCUl->Scale(6.72309e7/(2.49632e8 + 1.28757e8 + 6.72309e7));

        //TH1D *hEffPCU = (TH1D*) hEffPCUk->Clone(Form("hEffPCU_%s", sOpt.Data()));
        //hEffPCU->Add(hEffPCUh);
        //hEffPCU->Add(hEffPCUl);
