...
void UncerPLRatio(const TString sType = "Xi", const TString CentMin = "0", const TString CentMax = "100"){
  ...
  //===========================UESub
  auto hNJK = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtNJ_Lambda_%s%s", CentMin.Data(), CentMax.Data())); hNJK->SetName("hNJK");
  auto hOCK = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtOC_Lambda_%s%s", CentMin.Data(), CentMax.Data())); hOCK->SetName("hOCK");
  auto hPCK = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtPC_Lambda_%s%s", CentMin.Data(), CentMax.Data())); hPCK->SetName("hPCK");

  auto hNJA = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtNJ_AntiLa_%s%s", CentMin.Data(), CentMax.Data())); hNJA->SetName("hNJA");
  auto hOCA = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtOC_AntiLa_%s%s", CentMin.Data(), CentMax.Data())); hOCA->SetName("hOCA");
  auto hPCA = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtPC_AntiLa_%s%s", CentMin.Data(), CentMax.Data())); hPCA->SetName("hPCA");
  hNJK->Add(hNJA); hNJK->SetName("hNJK");
  hOCK->Add(hOCA); hOCK->SetName("hOCK");
  hPCK->Add(hPCA); hPCK->SetName("hPCK");

  TH1D* hNJ = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtNJ_%s_%s%s", sType.Data(), CentMin.Data(), CentMax.Data()));hNJ->SetName("hNJ");
  TH1D* hOC = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtOC_%s_%s%s", sType.Data(), CentMin.Data(), CentMax.Data()));hOC->SetName("hOC");
  TH1D* hPC = (TH1D*)GetTH1D(sPath, sFile2, "", Form("hPtPC_%s_%s%s", sType.Data(), CentMin.Data(), CentMax.Data()));hNJ->SetName("hPC");
  ...
  auto hJPt0K = (TH1D*)GetTH1D(sPath, sFile3, Form("Lambda_JetPt_%s%s", CentMin.Data(), CentMax.Data()), "hist_0"); hJPt0K->SetName("hJPt0K");
  auto hJPt1K = (TH1D*)GetTH1D(sPath, sFile3, Form("Lambda_JetPt_%s%s", CentMin.Data(), CentMax.Data()), "hist_1"); hJPt1K->SetName("hJPt1K");
  auto hJPtK00 =(TH1D*)GetTH1D(sPath, sFile3, Form("AntiLa_JetPt_%s%s", CentMin.Data(), CentMax.Data()), "hist_0"); hJPtK00->SetName("hJPtK00");
  auto hJPtK11 =(TH1D*)GetTH1D(sPath, sFile3, Form("AntiLa_JetPt_%s%s", CentMin.Data(), CentMax.Data()), "hist_1"); hJPtK11->SetName("hJPtKK11");
  hJPt0K->Add(hJPtK00);hJPt0K->SetName("hJPt0K");  hJPt1K->Add(hJPtK11);hJPt1K->SetName("hJPt1K");
  
  TH1D* hJPt0 = (TH1D*)GetTH1D(sPath, sFile3, Form("%s_JetPt_%s%s", sType.Data(), CentMin.Data(), CentMax.Data()), "hist_0"); hJPt0->SetName("hJPt0");
  TH1D* hJPt1 = (TH1D*)GetTH1D(sPath, sFile3, Form("%s_JetPt_%s%s", sType.Data(), CentMin.Data(), CentMax.Data()), "hist_1"); hJPt1->SetName("hJPt1");
  ...
  l->Write(Form("%s_toLRatio_%s%s",sType.Data(), CentMin.Data(), CentMax.Data()), TObject::kSingleKey);
  f->Close();
  ...
  tex->DrawLatex(0.15, 0.63, Form("#frac{#%s + #bar{#%s}}{#Lambda + #bar{#Lambda}} %s-%s%%", sType.Data(), sType.Data(), CentMin.Data(), CentMax.Data()));
  leg->Draw();
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/ThisAna/%s_toLRatio_%s%s.eps", sType.Data(), CentMin.Data(), CentMax.Data()));
  ...
  leg1->Draw();
  gStyle->SetOptStat("");
  can1->SaveAs(Form("./figure/Uncert/%s_toL_Uncert_%s%s.eps", sType.Data(), CentMin.Data(), CentMax.Data()));
  //DrawAliLogo(0.65, 0.90, 24, kTRUE);
  CanvasEnd(can1);
  return;
}
