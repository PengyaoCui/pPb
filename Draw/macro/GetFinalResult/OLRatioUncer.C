...
void OLRatioUncer(const TString CentMin = "0", const TString CentMax = "100"){
  ...
  auto hInR = (TH1D*)GetTH1D(sPath, CFile, CListIn, Form("hOLRatio_%s%s", CentMin.Data(), CentMax.Data())); hInR->SetName("hInR"); ;
  auto hJCR = (TH1D*)GetTH1D(sPath, CFile, CListJC, Form("hOLRatio_%s%s", CentMin.Data(), CentMax.Data())); hJCR->SetName("hJCR"); ;
  auto hPCR = (TH1D*)GetTH1D(sPath, CFile, CListPC, Form("hOLRatio_%s%s", CentMin.Data(), CentMax.Data())); hPCR->SetName("hUER"); ;
  auto hJER = (TH1D*)GetTH1D(sPath, CFile, CListJE, Form("hOLRatio_%s%s", CentMin.Data(), CentMax.Data())); hJER->SetName("hJER"); ;


  auto sListD =  Form("Lambda_sum_%s%s", CentMin.Data(), CentMax.Data());
  auto sList =  Form("Omega_%s%s", CentMin.Data(), CentMax.Data()); 
  //---------------------------InclUncer
  ...
  TH1D* hJPt0 = nullptr;
  TH1D* hJPt1 = nullptr;
  hJPt0 =(TH1D*)GetTH1D(sPath, sFile3, Form("Omega_JetPt_%s%s", CentMin.Data(), CentMax.Data()), "hist_0"); hJPt0->SetName("hJPt0");
  hJPt1 =(TH1D*)GetTH1D(sPath, sFile3, Form("Omega_JetPt_%s%s", CentMin.Data(), CentMax.Data()), "hist_1"); hJPt1->SetName("hJPt1");
  ...
  l->Write(Form("Omega_toLRatio_%s%s", CentMin.Data(), CentMax.Data()), TObject::kSingleKey);
  f->Close();
  ...
  leg->Draw();
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/Uncert/OLRatio_%s%s_JCUncert.pdf", CentMin.Data(), CentMax.Data()));
  can->SaveAs(Form("./figure/Uncert/OLRatio_%s%s_JCUncert.eps", CentMin.Data(), CentMax.Data()));
