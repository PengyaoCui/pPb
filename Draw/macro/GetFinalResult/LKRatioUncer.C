...
void LKRatioUncer(const TString CentMin = "0", const TString CentMax = "100"){
  ...
  auto hInR = (TH1D*)GetTH1D(sPath, CFile, CListIn, Form("hLKRatio_%s%s", CentMin.Data(), CentMax.Data())); hInR->SetName("hInR"); ;
  auto hJCR = (TH1D*)GetTH1D(sPath, CFile, CListJC, Form("hLKRatio_%s%s", CentMin.Data(), CentMax.Data())); hJCR->SetName("hJCR"); ;
  auto hPCR = (TH1D*)GetTH1D(sPath, CFile, CListPC, Form("hLKRatio_%s%s", CentMin.Data(), CentMax.Data())); hPCR->SetName("hUER"); ;
  auto hJER = (TH1D*)GetTH1D(sPath, CFile, CListJE, Form("hLKRatio_%s%s", CentMin.Data(), CentMax.Data())); hJER->SetName("hJER"); ;


  auto sListD =  Form("Kshort_%s%s", CentMin.Data(), CentMax.Data());
  auto sList =  Form("Lambda_sum_%s%s", CentMin.Data(), CentMax.Data()); 
  //---------------------------InclUncer
  ...
  for(Int_t i = 1; i<= hJPt0R->GetNbinsX(); i++){
    ...
    heJPtR->SetBinContent(i, TMath::Abs(dJPt0R-dJPt1R));
  }
  ...
  TFile *f = TFile::Open("./result/pPb.root", "UPDATE");
  TList *l = new TList();
  ...
  l->Write(Form("Lambda_sum_toKRatio_%s%s", CentMin.Data(), CentMax.Data()), TObject::kSingleKey);
  f->Close();
  ...
  tex->DrawLatex(0.15, 0.9, "p-Pb at #sqrt{s_{NN}} = 5.02 TeV");
  ...
  leg->Draw();
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/Uncert/LKRatio_%s%s_JCUncert.pdf", CentMin.Data(), CentMax.Data()));
  can->SaveAs(Form("./figure/Uncert/LKRatio_%s%s_JCUncert.eps", CentMin.Data(), CentMax.Data()));
  //DrawAliLogo(0.65, 0.90, 24, kTRUE);
  CanvasEnd(can);

  ofstream OutFile;
  OutFile.open(Form("./dat/LKRatioUncer_%s%s.dat", CentMin.Data(), CentMax.Data()));
  OutFile<<left<<setw(15)<<"LKRatio"<<"Centrality:"<<CentMin<<"-"<<CentMax<<endl;
