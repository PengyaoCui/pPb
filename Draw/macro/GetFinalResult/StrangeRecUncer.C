...
void StrangeRecUncer(const TString sType = "Kshort", const TString CentMin = "0", const TString CentMax = "100", const TString sUncertType = "Topological"){
  ...
  TH1D* hRatio = nullptr;
  TH1D* hU[nhist];
  h2 = (TH1D*)GetTH1D(Form("/home/cuipengyao/study/pPb/Local/ThisAna/Incl/%s", sType.Data()), sFile.Data(), List.Data(), sHist.Data());
  ...
  leg->Draw();
  gStyle->SetOptStat("");
  if(IsSaveCan) can->SaveAs(Form("./figure/Uncert/%s_%s_%s%s.pdf", sType.Data(), sUncertType.Data(), CentMin.Data(), CentMax.Data()));
  CanvasEnd(can);
  return;
}
