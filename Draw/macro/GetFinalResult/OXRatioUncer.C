...
void OXRatioUncer(const TString CentMin = "0", const TString CentMax = "100"){
  ...
  auto hInR = (TH1D*)GetTH1D(sPath, CFile, CListIn, Form("hOXRatio_%s%s", CentMin.Data(), CentMax.Data())); hInR->SetName("hInR"); ;
  auto hJCR = (TH1D*)GetTH1D(sPath, CFile, CListJC, Form("hOXRatio_%s%s", CentMin.Data(), CentMax.Data())); hJCR->SetName("hJCR"); ;
  auto hPCR = (TH1D*)GetTH1D(sPath, CFile, CListPC, Form("hOXRatio_%s%s", CentMin.Data(), CentMax.Data())); hPCR->SetName("hUER"); ;
  auto hJER = (TH1D*)GetTH1D(sPath, CFile, CListJE, Form("hOXRatio_%s%s", CentMin.Data(), CentMax.Data())); hJER->SetName("hJER"); ;


  auto sListD =  Form("Xi_%s%s", CentMin.Data(), CentMax.Data());
  ...
  l->Write(Form("Omega_toXRatio_%s%s", CentMin.Data(), CentMax.Data()), TObject::kSingleKey);
  f->Close();
  ...
  leg->Draw();
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/Uncert/OXRatio_%s%s_JCUncert.pdf", CentMin.Data(), CentMax.Data()));
  can->SaveAs(Form("./figure/Uncert/OXRatio_%s%s_JCUncert.eps", CentMin.Data(), CentMax.Data()));
  //DrawAliLogo(0.65, 0.90, 24, kTRUE);
  CanvasEnd(can);

  ofstream OutFile;
  OutFile.open(Form("./dat/OXRatioUncer_%s%s.dat", CentMin.Data(), CentMax.Data()));
  OutFile<<left<<setw(15)<<"OXRatio"<<"Centrality:"<<CentMin<<"-"<<CentMax<<endl;
