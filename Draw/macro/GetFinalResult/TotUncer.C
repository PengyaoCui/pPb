...
void TotUncer(const TString sType = "Lambda", const TString CentMin = "0", const TString CentMax = "100"){
  
  auto sPath = "./result"; 
  auto sCFile = "CentVal.root";
 
  auto sCInclList = "listIn"; 
  auto sCInclHist = Form("h%s_%s%s", sType.Data(), CentMin.Data(), CentMax.Data());
  ...
  auto sCJCHist = Form("h%s_%s%s", sType.Data(), CentMin.Data(), CentMax.Data());
  ...
  ofstream OutFile;
  OutFile.open(Form("./dat/TotUncer_%s_%s%s.dat", sType.Data(), CentMin.Data(), CentMax.Data()));
  ...
  leg->Draw();
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/Uncert/%s_JCUncert_%s%s.pdf", sType.Data(), CentMin.Data(), CentMax.Data()));
  can->SaveAs(Form("./figure/Uncert/%s_JCUncert_%s%s.eps", sType.Data(), CentMin.Data(), CentMax.Data()));
