...
void FinalSpect(const TString sType = "AntiLa", const TString CentMin = "0", const TString CentMax = "100", const Bool_t IsSum = kFALSE){
  ...
  auto sCHist = Form("h%s_%s%s", sType.Data(), CentMin.Data(), CentMax.Data());
  ...
  if(sType == "Lambda" && IsSum){
    auto hAIncl1 = (TH1D*)GetTH1D(sPath, sCFile,sCInclList, Form("hAntiLa_%s%s", CentMin.Data(), CentMax.Data())); hAIncl1->SetName("AInclCen");
    auto hAJE1   = (TH1D*)GetTH1D(sPath, sCFile,sCJEList,   Form("hAntiLa_%s%s", CentMin.Data(), CentMax.Data())); hAJE1->SetName("AJECen");
    auto hAUE1   = (TH1D*)GetTH1D(sPath, sCFile,sCPCList,   Form("hAntiLa_%s%s", CentMin.Data(), CentMax.Data())); hAUE1->SetName("AUECen");
    auto hAJC1   = (TH1D*)GetTH1D(sPath, sCFile,sCJCList,   Form("hAntiLa_%s%s", CentMin.Data(), CentMax.Data())); hAJC1->SetName("AJCCen");
    auto hAOC1   = (TH1D*)GetTH1D(sPath, sCFile,sCOCList,   Form("hAntiLa_%s%s", CentMin.Data(), CentMax.Data())); hAOC1->SetName("AOCCen");
    auto hANJ1   = (TH1D*)GetTH1D(sPath, sCFile,sCNJList,   Form("hAntiLa_%s%s", CentMin.Data(), CentMax.Data())); hANJ1->SetName("ANJCen");
    ...
    auto hAJC2 = (TH1D*)GetTH1D(sPath, sEFile, Form("AntiLa_UncerTot_%s%s", CentMin.Data(), CentMax.Data()), sEJCHist); hAJC2->SetName("AJCErr");
  ...
  l->Add(gOC);
  l->Add(gNJ);
  if(!IsSum)l->Write(Form("%s_%s%s",sType.Data(), CentMin.Data(), CentMax.Data()), TObject::kSingleKey);
  if(IsSum)l->Write(Form("%s_sum_%s%s",sType.Data(), CentMin.Data(), CentMax.Data()), TObject::kSingleKey);
  f->Close();
  //-----------------------------------
  tex =  new TLatex();
  tex->SetNDC();
  ...
  if(sType == "Kshort"){
    tex->DrawLatex(0.15, 0.3, Form("K^{0}_{S}   %s-%s%%", CentMin.Data(), CentMax.Data()));
    tex->DrawLatex(0.15, 0.2, "|#it{#eta}_{K^{0}_{S}}| < 0.75, #Delta#it{R}(K^{0}_{S}, jet) < 0.4");
  }
  if(sType == "AntiLa"){
    tex->DrawLatex(0.15, 0.3, Form("#bar{#Lambda}   %s-%s%%", CentMin.Data(), CentMax.Data()));
    tex->DrawLatex(0.15, 0.2, "|#it{#eta}_{#bar{#Lambda}}| < 0.75, #Delta#it{R}(#bar{#Lambda}, jet) < 0.4");
  }
  if(sType == "Lambda" && IsSum){
    tex->DrawLatex(0.15, 0.3, Form("#Lambda + #bar{#Lambda}   %s-%s%%", CentMin.Data(), CentMax.Data()));
    tex->DrawLatex(0.15, 0.2, "|#it{#eta}_{#Lambda}| < 0.75, #Delta#it{R}(#Lambda, jet) < 0.4");
  }
  if((sType == "Lambda" && !IsSum) || sType == "Xi" || sType == "Omega"){
    tex->DrawLatex(0.15, 0.3, Form("#%s  %s-%s%%", sType.Data(), CentMin.Data(), CentMax.Data()));
  ...
  leg->Draw();
  gStyle->SetOptStat("");
  //if(!IsSum)can->SaveAs(Form("./figure/ThisAna/%s_wSysError_%s%s.eps", sType.Data(), CentMin.Data(), CentMax.Data()));
  //if(IsSum)can->SaveAs(Form("./figure/ThisAna/%s_sum_wSysError_%s%s.eps", sType.Data(), CentMin.Data(), CentMax.Data()));
