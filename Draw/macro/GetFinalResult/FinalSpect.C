...
void FinalSpect(const TString sType = "AntiLa", const TString CentMin = "0", const TString CentMax = "100", const Bool_t IsSum = kFALSE){
  ...
  if(sType == "Lambda" && IsSum){
    ...
    auto hAP21   = (TH1D*)GetTH1D(sPath, sCFile,sCP2List,   Form("hAntiLa_%s%s", CentMin.Data(), CentMax.Data())); hAP21->SetName("AP2Cen");
    auto hAP61   = (TH1D*)GetTH1D(sPath, sCFile,sCP6List,   Form("hAntiLa_%s%s", CentMin.Data(), CentMax.Data())); hAP61->SetName("AP6Cen");
