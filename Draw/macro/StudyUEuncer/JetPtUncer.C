...
void JetPtUncer(const TString sType = "Kshort", const TString CentMin = "0", const TString CentMax = "100"){
               
  TString sCut[] = {"Tight", "Loose"}; 

  TString sPath = "/home/cuipengyao/study/pPb/Local/ThisAna/Uncertainty/JE/JetPt";
  if(sType == "Omega") sPath = "/home/cuipengyao/study/pPb/Local/ThisAna/Uncertainty/JE_Omega/JetPt";
  ...
  TH1D* hRatio = nullptr;
  TH1D* hU[nhist+1];
  h2 = (TH1D*)GetTH1D(Form("/home/cuipengyao/study/pPb/Local/ThisAna/JE/%s", sType.Data()), sFile.Data(), List.Data(), sHist.Data());
  if(sType == "Omega")h2 = (TH1D*)GetTH1D(Form("/home/cuipengyao/study/pPb/Local/ThisAna/JE_OmegaBins/%s", sType.Data()), sFile.Data(), List.Data(), sHist.Data());
  h2->SetName("Default");
  h2->SetTitle("");
  NormHistBinWidth(h2);  
  ...
  leg->Draw();
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/Uncert/%s_JetPt_%s%s.pdf", sType.Data(), CentMin.Data(), CentMax.Data()));
  CanvasEnd(can);
  return;
}
