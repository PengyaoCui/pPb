...
void Ratio(){
  ...
  h->GetYaxis()->SetRangeUser(1e5, 1e8);
  h->SetMarkerSize(1);
  ...
  tex->DrawLatex(0.2, 0.9, "p-Pb at #sqrt{s}_{NN} = 5.02 TeV");
  tex->DrawLatex(0.2, 0.8, "#Omega corrected yield");
  leg->Draw();
  gStyle->SetOptStat("");
  can->SaveAs(Form("./figure/Omega_LowPtbin_ppb.eps"));
  can->SaveAs(Form("./figure/Omega_LowPtbin_ppb.pdf"));
  CanvasEnd(can);
  return;
}
