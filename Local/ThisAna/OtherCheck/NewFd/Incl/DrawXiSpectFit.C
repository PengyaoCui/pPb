...
void FitXi(TString sCone = "Incl", TString sCent = "0100");
void DrawXiSpectFit(){
  FitXi("Incl", "0100");
}
void FitXi(TString sCone = "Incl", TString sCent = "0100"){
  ...
  h0->GetXaxis()->SetRangeUser(0.6, 12.);
  h0->GetYaxis()->SetRangeUser(1e-6, 1e-1);
  h0->SetTitle("");
  SetFrame(h0, "#it{p}_{T}(GeV/#it{c})", "d#rho/d#it{p}_{T}");
  DrawHisto(h0, cLine[0], sMark[0], "same");

  
  TH1D* h1;
  TString sList;
  if(sCone == "Incl") sList = Form("listResults_Xi_Default_%s", sCent.Data());
  if(sCone != "Incl") sList = Form("listResults_Xi_Default_%s_%s", sCone.Data(), sCent.Data());
  ...
  TFile *file = TFile::Open("XiSpect_Draw.root", "UPDATE");
  file->cd();
  fLevyFitFunc->Write(Form("FitFun_%s_%s", sCone.Data(), sCent.Data()), TObject::kSingleKey);
  h2->Write(Form("%s_Xi_%s", sCone.Data(), sCent.Data()), TObject::kSingleKey);
  ...
  SetLegend(leg);
  tex->DrawLatex(0.16, 0.91, "Inclusive #Xi spectrum");
